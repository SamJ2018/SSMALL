...
public class CartServiceImpl implements CartService {
    ...
    public List<Cart> addGoodsToCartList(List<Cart> cartList, Long itemId, Integer num) {
        ...
        TbItem item = itemMapper.selectByPrimaryKey(itemId);//SKU对象
        if (item == null) {
        ...
        if (cart == null) {  //4、如果购物车列表中不存在该商家的购物车
            ...
            if (orderItem == null) {
                ...
                orderItem.setTotalFee(new BigDecimal(orderItem.getNum() * orderItem.getPrice().doubleValue()));//金额

                if (orderItem.getNum() <= 0) {
                    cart.getOrderItemList().remove(orderItem);//+ - 当明细的数量小于等于0，移除明细
                }

                if (cart.getOrderItemList().size() == 0) { //购物车明细都没了,在购物车列表移除此购物车
    ...
    public List<Cart> mergeCartList(List<Cart> cartList1, List<Cart> cartList2) {
        ...
        for (Cart cart : cartList2) {//遍历第二个集合，往第一个集合中加
    ...
    public TbOrderItem searchOrderItemByItemId(List<TbOrderItem> orderItemList, Long itemId) {
        for (TbOrderItem orderItem : orderItemList) {
            if (orderItem.getItemId().longValue() == itemId.longValue()) {//注意这里是Long，需要基本数据类型才可以==比较
                return orderItem;
            }
        }
        return null;
